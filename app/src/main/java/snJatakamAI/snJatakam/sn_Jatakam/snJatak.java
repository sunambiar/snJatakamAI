...
public class snJatak {
  ...
  Map mapJson = null, mapJsonA = null, mapJsonB = null, mapJsonC = null;
 ...
 /*---------------------------------------------------------------*/
  ...
  private void misc() {
    ...
    hWrite("</TD><TD>");
    pS.println(V.nak[na]);
    mapJson.put("name", V.nak[na].trim());
    ...
    hWrite("</TD><TD>");
    pS.println(V.ras[ra]);
    mapJson.put("name", V.ras[ra].trim());
    ...
    mapJson = new LinkedHashMap(2);
    mapJson.put("val", pak);
    mapJson.put("name", V.paksham[pak].trim());
    ...
    mapJson.put("name", V.karanam[V.karId[karanam]].trim());
  ...
  private void misc(long d, long m, long y, long j) {
    ...
    sun_rise += (sun_rise < 0) ? 24 : (sun_rise >= 24) ? -24 : 0; 
    ...
    sun_set += (sun_set < 0) ? 24 : (sun_set >= 24) ? -24 : 0;
  ...
  private void prplnt() {
    ...
    for (i = 0; i <= nplnt; i++) {
      ...
      if (html) {
        ...
        mapJsonA.put("rasi_id", (int) xx + 1);
        mapJsonA.put("rasi_name", V.ras[(int) xx].trim());
      ...
      mapJsonA = new LinkedHashMap(2);
      mapJsonA.put("val", b);
      mapJsonA.put("name", V.nak[b].trim());
  ...
  private void bhav() {
    ...
    mapJson.put("name", ayana_opts[ayana_opt]);
    ...
    for (i = 0; i < nrasi; i++) {
      ...
      for (int kk, k = 1; k <= nplnt; k++) {
        ...
        if (fnd || (f3[i] <= plnt[0][k] && plnt[0][k] <= f3[kk])) {
          ...
          mapJsonA = new LinkedHashMap(2);
          mapJsonA.put("id", k);
          mapJsonA.put("name", V.grahalong[k].trim());
          jArr1.put(mapJsonA);
        }
      }
  ...
  private void vgcomp(String y1, double x0, int t) {
    ...
    if (jArr != null) {
      ...
      //mapJson.put("vargas_str", (Arrays.toString(varga[t])));
      mapJson.put("vargas", new JSONArray(Arrays.toString(varga[t])));
      jArr.put(mapJson);
    }
    pS.println();
    hWrite("</TD></TR>");
  ...
  private void vgcalc(int m) {
    ...
    if (!html) {
      ...
      for (i = 0; i < 12; i++) {
        ...
        mapJson.put(rasiArrangement[i], jArr);
        if (i == 4) {
  ...
  private void ashtakPrint(String x1) {
    int i;
    hWrite("<TR><TD>");
    mapJsonA = new LinkedHashMap(nrasi);
    pS.print(x1 + " ");
    for (i = 1; i <= nrasi; i++) {
  ...
  private void ashtak() {
    ...
    mapJson.put("total", mapJsonA);
    jObj.put("ashtaka_varga", mapJson);
    pageHeader();
  }
  ...
  private void vimst(int detl, int d1, int m1, int y1) {
    ...
    for (c = q; c <= q + 8; c++) {
      ...
      mapJson.put(V.dL[c].trim(), mapJsonA);
      for (d = 0; d <= 8; d++) {
        n = c + d;
        n %= 9;     //  if (n > 8) n -=9;
        if (detl == 0) {
          ...
          mapJsonB = new LinkedHashMap(3);
          mapJsonA.put(V.dL[n].trim(), mapJsonB);
          mapJsonB.put("uptoDate", fmt.format(t) + "-" + V.mth[r - 1] + "-" + p);
          mapJsonB.put("uptoDate_ymd", p + "-" + fmt.format(r) + "-" + fmt.format(t));
          hWrite(sTDTDc);
          pS.println(snDate.dateDiff(new snDate(p, r, t), snDOB).ymd());
          mapJsonB.put("uptoAge", snDate.dateDiff(new snDate(p, r, t), snDOB).ymd());
          hWrite("</TD></TD>");
          line++;
          if (line > 60) {
            hWrite("</TABLE>");
          ...
          for (e = 0; e <= 8; e++) {
            ...
            pS.print("  " + V.dL[n]);
            hWrite("</TD><TD>");
            mapJsonB = new LinkedHashMap(7);  // 7 AntarDasas 
            mapJsonA.put(V.dL[n].trim(), mapJsonB);
            pS.print("  " + V.dL[g]);
            hWrite(sTDTDc);
            mapJsonC = new LinkedHashMap(3);
            mapJsonB.put(V.dL[g].trim(), mapJsonC);
            /*
               pS.print("   " + fmt.format(t)); hWrite(sTDTDc);
               pS.print("   " + fmt.format(r)); hWrite(sTDTDc);
             ...
             */
            pS.print(fmt.format(t) + "-" + V.mth[r - 1] + "-" + p + " ");
            mapJsonC.put("uptoDate", fmt.format(t) + "-" + V.mth[r - 1] + "-" + p);
            mapJsonC.put("uptoDate_ymd", p + "-" + fmt.format(r) + "-" + fmt.format(t));
            hWrite(sTDTDc);
            pS.println(snDate.dateDiff(new snDate(p, r, t), snDOB).ymd());
            mapJsonC.put("uptoAge", snDate.dateDiff(new snDate(p, r, t), snDOB).ymd());
            hWrite("</TD></TD>");
            line++;
            if (line > 60) {
              hWrite("</TABLE>");
  ...
  public void snJatak(String args[]) {
    ...
    try {
      ...
      if (!argsPassed) {
        ...
        System.out.println("[VH] serVlet-HTML");
        System.out.flush();
      ...
      hWrite("<STRONG>");
      pS.println(sex);
      jObj.put("sex", sex.trim());
      hWrite("</TD></TR><TR><TD>");
      pS.print("Date of Birth  : ");
      hWrite("</TD><TD>");
      hWrite("<STRONG>");
      pS.print("" + fmt.format(d) + "-" + V.mth[(int) m - 1] + "-" + y);
      jObj.put("dob", "" + fmt.format(d) + "-" + V.mth[(int) m - 1] + "-" + y);
      jObj.put("dob_ymd", "" + y + "-" + fmt.format(m) + "-" + fmt.format(d));
      hWrite("</TD>");
      hWrite("<TD width=75>");
      hWrite("<STRONG>");
      //d_o_w = V.day[(int)j];
      snE.dow_No = (short) j;
      snE.dow = V.day[(int) j];
      jObj.put("day_of_week", snE.dow);
      jObj.put("dow", snE.dow_No);
