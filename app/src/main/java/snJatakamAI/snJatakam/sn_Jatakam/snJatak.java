...
public class snJatak {
     ...
     */
     ...
     private static double getHour (double sphutam,double ad,
	   ...
	   //}
       if ((sphutam) > 360.0) sphutam -= 360.0;
       else if ((sphutam) < 0.0) sphutam += 360.0;
       ...
       if (sphutnew >= 360.0) sphutnew -= 360.0;
       else if (sphutnew < 0.0) sphutnew += 360.0;
       for (kkk=0; kkk<15 && Math.abs(sphutnew - sphutam) > one_sec; kkk++) {
        do {
         ...
         if (time_diff >= 24.0) time_diff -= 24.0;
         if (time_diff <= -24.0) time_diff += 24.0;
         //if (time_diff > 12.0) time_diff -= 24.0;
         hr += time_diff;
         if (hr >= 24.0) hr -= 24;
     ...
     private static double setSayana (double aya_val) {
       for (int i = 1; i <= nplnt; i++) {
         ...
         if (plnt[2][i] < 0.0)   plnt[2][i] += 360.0;
         if (plnt[2][i] >= 360.0) plnt[2][i] -= 360.0;
         plnt[1][i] = plnt[1][i] + aya_val;  // Sayana
         if (plnt[1][i] < 0.0)   plnt[1][i] += 360.0;
         if (plnt[1][i] >= 360.0) plnt[1][i] -= 360.0;
     ...
     private static double applyAyan (double aya_val) {
       for (int i = 1; i < nplnt; i++) {
         ...
         if (plnt[0][i] < 0.0)   plnt[0][i] += 360.0;
         if (plnt[0][i] > 360.0) plnt[0][i] -= 360.0;
       }
       plnt[0][0] = aya_val;
       return (aya_val);
     }
     ...
     private static double ayan (int ayan_opt,
       ...
       switch (ayan_opt) {
         case 1  : ret = 21.013972 + 1.398191*b6;  /// BVRaman
                   break;
         case 2  : /*
				   ...
				   */
                   ...
                   break;
         case 3  : ret = aya_val;   // User specified
                   break;
         default : ret = 22.460148 + 1.396042*b6 + 3.08e-4*b6*b6;
                   break;     // NCLahiri
       }
       return (ret);
     }
     ...
     private static double long_planet (double pg, double ph, double pp,
       ...
       if (pc < 0.0) pm += 180.0;
       else if (pd < 0.0) pm += 360.0;
       while (pm < 0.0) pm += 360.0;
       return (pm);
     }
     /*---------------------------------------------------------------*/
     ...
     private static void gulikan (long d, long m, long y) {
       ...
       if (plnt[0][13] >= 360.0) plnt[0][13] -= 360.0;
       else if (plnt[0][13] < 0) plnt[0][13] += 360.0;
       //pS.println("Gulika(Traditnl.Method)="+DMS.dms(plnt[0][13]));
       //pS.println(" Gulika Rise = " + gul_rise);
       //pS.println(" Day = " + j + " " +V.day[(int)j]);

       plnt[2][13] = plnt[0][13] + ayanamsa;    // Sayana
       if (plnt[2][13] >= 360.0) plnt[2][13] -= 360.0;
       else if (plnt[2][13] < 0.0) plnt[2][13] += 360.0;
     ...
     private static void misc () {
       ...
       */
       ...
       if (c > 180.0) c -= 360.0;
       csdiff = (c - s);
       if (csdiff < 0.0) csdiff += 360.0;
       if (csdiff == 180.0 || csdiff == 0.0) pak = 2;
       else if (csdiff < 180.0) pak = 1;
       else pak = 0;
       ...
       hWrite("<TR><TD>");
       pS.print("Tithi      : "); hWrite("</TD><TD>");
       pS.print(round(tt[0],2));  hWrite("</TD><TD>");
       if (!html) pS.print("          \t\t");  hWrite("</TD><TD>");
       pS.print("Tithi Name     : "); hWrite("</TD><TD>");
       pS.println( V.tit[ti]);           hWrite("</TD></TR><TR><TD>");
       pS.print("Nakshatra  : ");     hWrite("</TD><TD>");
       //pS.print(nakshatra);      hWrite("</TD><TD>");
       pS.print(round(tt[1],2));      hWrite("</TD><TD>");
       if (!html) pS.print("\t");
       pS.print("(" + part + " Paada)"); hWrite("</TD><TD>");
       if (!html) pS.print("\t");
       pS.print("Nakshatra Name : "); hWrite("</TD><TD>");
       pS.println( V.nak[na]);           hWrite("</TD></TR><TR><TD>");

       yogam = (short)yo;
       pS.print("Yoga       : ");     hWrite("</TD><TD>");
       pS.print(round(tt[2],2));      hWrite("</TD><TD>");
       if (!html) pS.print("          \t\t");  hWrite("</TD><TD>");
       pS.print("Yoga Name      : "); hWrite("</TD><TD>");
       pS.println( V.yog[yogam]);           hWrite("</TD></TR><TR><TD>");

       pS.print("Raasi      : ");     hWrite("</TD><TD>");
       pS.print((int)tt[3] + "S " + DMS.dms((tt[3] - (int)tt[3])*30));
       hWrite("</TD><TD>");
       if (!html) pS.print("\t\t");   hWrite("</TD><TD>");
       pS.print("Raasi Name     : "); hWrite("</TD><TD>");
       pS.println(V.ras[ra]);           hWrite("</TD></TR><TR><TD>");

       hWrite("</TD></TR><TR><TD>");
       pS.print("Ganam      : ");         hWrite("</TD><TD>");
       pS.print( V.ganam[ V.gnmId[na]]);    hWrite("</TD><TD>");
       if (!html) pS.print("\t\t\t");     hWrite("</TD><TD>");
       pS.print("Devata     : ");         hWrite("</TD><TD>");
       pS.println( V.devata[na]);        hWrite("</TD></TR><TR><TD>");
       pS.print("Vriksham   : ");       hWrite("</TD><TD>");
       pS.print( V.vriksha[na]);         hWrite("</TD><TD>");
       if (!html) pS.print("\t\t\t");     hWrite("</TD><TD>");
       pS.print("Yoni       : ");           hWrite("</TD><TD>");
       pS.println( V.yoni[ V.yonId[na]]);   hWrite("</TD></TR><TR><TD>");
       pS.print("Bhootam    : ");        hWrite("</TD><TD>");
       pS.print( V.bhootam[bh]);         hWrite("</TD><TD>");
       if (!html) pS.print("\t\t\t");     hWrite("</TD><TD>");
       pS.print("Mrigam     : ");         hWrite("</TD><TD>");
       pS.println( V.mrigam[na]);        hWrite("</TD></TR><TR><TD>");
       pS.print("Pakshi     : ");         hWrite("</TD><TD>");
       pS.print( V.pakshi[pk]);          hWrite("</TD><TD>");
       if (!html) pS.print("\t\t\t");     hWrite("</TD><TD>");
       pS.print("Paksham    : ");        hWrite("</TD><TD>");
       pS.println( V.paksham[pak]);      hWrite("</TD></TR><TR><TD>");
       pS.print("Karanam    : ");        hWrite("</TD><TD>");
       //System.out.println("Kar=" + karanam);
     ...
     private void misc (long d, long m, long y, long j) {
       ...
       hWrite("<TR><TD>");
       pS.print("Sun rise   : ");     hWrite("</TD><TD>");
       pS.print(HMS.hms(sun_rise));   hWrite("&times;</TD><TD>");
       pS.println(" ("+HMS.hms(sun_riseA)+")");hWrite("&copy;</TD></TR><TR><TD>");
       ...
       pS.print("Noon       : ");     hWrite("</TD><TD>");
       pS.print(HMS.hms(noon));  hWrite("&times;</TD><TD>");
       pS.println(" ("+HMS.hms(noonA)+")");
       hWrite("&copy;</TD></TR><TR><TD>");

       //System.out.println("Sun_Set = "+HMS.hms(sun_set));
       pS.print("Sun set    : ");     hWrite("</TD><TD>");
       pS.print(HMS.hms(sun_set));    hWrite("&times;</TD><TD>");
       pS.println(" ("+HMS.hms(sun_setA)+")");
       hWrite("&copy;</TD></TR><TR><TD>");

       pS.print("Dina-Maanam: ");     hWrite("</TD><TD>");
       pS.print(HMS.hms(sun_set-sun_rise));    hWrite("&times;</TD><TD>");
       ...
       snE.sunset = sun_set;
       pS.print("Hindu Day of Birth  : "); hWrite("</TD><TD>");
       pS.println(V.day[hj]);  hWrite("</TD><TD>");
       //hWrite(snE.getMalayalamEra(d_o_b,plnt[0][1],
       hWrite("</TD></TR></TABLE>");

       hWrite("<TABLE><TR><TD>");
       hWrite("Abhjith Muhurtham: ");   hWrite("</TD><TD>");
     ...
     private static void prplnt () {
       ...
       lagn = lagnam();
       lagnS = (lagn + ayanamsa);
       if (lagnS > 360.0) lagnS -= 360.0;
       else if (lagnS < 0.0) lagnS += 360.0;
       for (i=0; i<=nplnt; i++) {
         ...
         hWrite("<TR><TD>");
         pS.print(V.grahalong[i]);    hWrite(sTDTDj);
         if (!html) pS.print("      ");
         if (html) {
            xx = aa/30.0;
            if (xx >= 12.0) xx -= 12.0;
            ...
            xx = (aa + ayanamsa)/30.0;
            if (xx >= 12.0) xx -= 12.0;
            while (xx < 0.0) xx += 12.0;
            //pS.print((int)xx+" S " + DMS.dms(30.0*fract(xx)));
            pS.print(V.ras[(int)xx] +" "+ DMS.dms(30.0*fract(xx)));
            hWrite(sTDTDc);
         }
         pS.print(DMS.dms(aa));     hWrite(sTDTDc);
         if (!html) pS.print(" (");
         /*
         xx = (aa + ayanamsa);
         ...
         */
         pS.print(DMS.dms(aaS)); hWrite(sTDTDc);
         if (!html) pS.print(")   \t" );
         if (!html) {
           pS.print(V.ras[a]);   //       hWrite(sTDTDc);
         }
         pS.print(" (" + c + ") "); hWrite(sTDTDc);
         pS.print( V.nak[b]);       hWrite(sTDTDc);
         pS.println(pp);            hWrite("</TD></TR>");
       }
       hWrite("<TR><TD>");
       pS.print("Kunda Lagnam");  hWrite(sTDTDc);
       klagn = kunda_Amsam(lagn);
       pS.print(DMS.dms(klagn));           hWrite(sTDTDc);
       double klagdiff = kunda_lagna_diff(lagn, plnt[0][7]);
       pS.print(DMS.dms(klagdiff));        hWrite(sTDTDc);
       hWrite("</TD></TR><TR><TD>");
       pS.print("Actual Lagnam");  hWrite(sTDTDc);
       pS.print(DMS.dms(lagn + klagdiff)); hWrite(sTDTDc);
       hWrite("</TD></TR>");
       /*
       hWrite("<TR><TD>");
     ...
     private static double kunda_lagna_diff (double lag, double moon) {
       ...
       */
       ...
       if ((long)round((k1 - moon),0) == 0) corr = c1;
       else if ((long)round((k2 - moon),0) == 0) corr = c2;
       else if ((long)round((k3 - moon),0) == 0) corr = -c1;
       else if ((long)round((k4 - moon),0) == 0) corr = -c2;
       else if (120 == ((long)round(Math.abs(k1 - moon),0))) corr = c1;
       else if (120 == ((long)round(Math.abs(k2 - moon),0))) corr = c2;
       else if (120 == ((long)round(Math.abs(k3 - moon),0))) corr = -c1;
       else if (120 == ((long)round(Math.abs(k4 - moon),0))) corr = -c2;
     ...
     private static double bhavspl (double sobliq, double a0, double c0) {
       ...
       */
       ...
       g0 /= z2;
       if (chandraHariAyana) {
		 if (b0 >= 180.0) g0 += 270.0;
		 else g0 += 90;
	   } else {
         if (g0 < 0.0) g0 += 180.0;
         if (b0 - g0 > 75.0) g0 += 180.0;
	   }
       g0 -= r0;
       if (g0 < 0.0) g0 += 360.0;
       if (g0 > 360.0) g0 -= 360.0;
     ...
     private static void pageHeader () {
       ...
       page++;
       pS.print("Jatakam of "); hWrite("&nbsp;<U>");
       pS.print(name);
       hWrite("</U></TD><TD width=400 align=right>");
       if (!html) pS.print("\t\t\t ");
       if (!html) pS.println("Page : " + page);
       hWrite("</TD></TR></TABLE>");
       /*
       if (!html) {
         pS.println("\f");
     ...
     private static void bhav () {
       ...
       for (i=1; i < nrasi; i++) {
         f3[i] = (f2[i-1] + f2[i]) / 2;
         if (f2[i] < f2[i-1]) f3[i] += 180.0;
         if (f3[i] >= 360.0) f3[i] -= 360.0;
       }
       hWrite("<TABLE border=2><TR><TD>");
       pS.print("Ayanamsa      : ");   hWrite("</TD><TD>");
       pS.print(DMS.dms(plnt[0][0]));  hWrite("</TD><TD>");
       pS.println("(" + ayana_opts[ayana_opt] + ")" );
       hWrite("</TD></TR><TR><TD>");
       pS.print("Obliquity     : ");   hWrite("</TD><TD>");
       pS.println(DMS.dms(obliq));     hWrite("</TD></TR><TR><TD>");
       pS.print("Siderial Time : ");   hWrite("</TD><TD>");
       pS.println(HMS.hms(sidtime));   hWrite("</TD></TR></TABLE>");
       hWrite("<HR><TABLE><TR><TD>");
       if (!html) pS.println(dashL);
       hWrite("</TD></TR><TR><TD>");
       ...
       pS.print(" Bhava ");            hWrite("</TD><TD>");
       if (!html) pS.print("       ");
       pS.print("Bhava.Begin ");       hWrite("</TD><TD>");
       if (!html) pS.print("       ");
       pS.print("Mid-Bhava  ");      hWrite("</TD><TD align=center>");
       pS.println("\tGrahas   ");      hWrite("</TD></TR>");
       if (!html) pS.println(dashL);
       for (i=0; i < nrasi; i++) {
         hWrite("<TR><TD align=center>");
         if (!html) pS.print("   ");
         pS.print((i+1));              hWrite("</TD><TD>");
         if (!html) pS.print("\t\t");
         pS.print(DMS.dms(f3[i]));     hWrite("</TD><TD>");
         if (!html) pS.print("\t");
         pS.print(DMS.dms(f2[i]));   hWrite("</TD><TD>");
         if (!html) pS.print("\t");
       ...
       hWrite("</TABLE>");
       hWrite("<TABLE><TR><TD>");
       if (!html) pS.println(dashL);  hWrite("</TD></TR></TABLE>");
       pageHeader();
     }
     /*---------------------------------------------------------------*/
     private static void saptavg () {
       ...
       pS.println("Div.60 ");
       */
       pS.print("Graham ");   hWrite("</TD><TD>");
       pS.print("  x 1  ");   hWrite("</TD><TD>");
       pS.print("  x 3  ");   hWrite("</TD><TD>");
       pS.print("  x 7  ");   hWrite("</TD><TD>");
       pS.print("  x 9  ");   hWrite("</TD><TD>");
       if (chandraHariAyana) {
         pS.print("  x 10 ");   hWrite("</TD><TD>");
	   } else {
         pS.print("Div.10 ");   hWrite("</TD><TD>");
	   }
       pS.print("  x 12 ");   hWrite("</TD><TD>");
       pS.print("  x 16 ");   hWrite("</TD><TD>");
       pS.print("  x 30 ");   hWrite("</TD><TD>");
       pS.print("  x 60 ");   hWrite("</TD><TD>");
       pS.print("  x 81 ");   pS.println();
       hWrite("</TD></TR><TR><TD></B>");
       if (!html) pS.println(dashL);
       vgcomp( V.grahalong[0], f2[0], 0); // Lagna
       for (i=1; i<=nplnt; i++) vgcomp( V.grahalong[i], plnt[0][i], i);
       hWrite("</TD></TR></TABLE>");
       hWrite("<TABLE><TR><TD>");
       if (!html) pS.println(dashL);  hWrite("</TD></TR><TR><TD>");
       pS.println();       hWrite("</TD></TR></TABLE>");
     }
     /*---------------------------------------------------------------*/
     private void listPaapas () {
       ...
       if (html || argsPassed) {
         ...
         for (i = 0; i < paapFrom.length; i++) {
           for (j = 0; j < totidx; j++) {
             ...
             for (k = 0; k < paapHouse.length; k++) {
               if (pos == paapHouse[k]) paapCount[i][j] = 1;
             }
           ...
           for (jj = totidx+1, j=0; jj < grndtotidx; jj++, j++) {
             ...
             paapCount[i][jj] = 0;
             for (k = 0; k < paapHouse.length; k++) {
               if (pos == paapHouse[k]) paapCount[i][jj] = 1;
             }
         ...
         for (i = 0; i < paapGrahA.length; i++) {
           pS.print( V.grahalong[paapGrahA[i]]);    hWrite("</TD><TD>");
         }
         pS.print("<-- Total -->");    hWrite("</TD><TD>");
         for (i = 0; i < paapGrahB.length; i++) {
           pS.print( V.grahalong[paapGrahB[i]]);    hWrite("</TD><TD>");
         }
         pS.print("<- Grand-Total ->"); hWrite("</TD></TR></B>");
         for (i = 0; i < paapFrom.length; i++) {
           hWrite("<TR align=center><TD>");
           pS.print( V.grahalong[paapFrom[i]]); hWrite("</TD>");
           for (j = 0; j <= grndtotidx; j++) {
             hWrite("<TD>");
             if (paapCount[i][j] > 0) pS.print(paapCount[i][j]);
             else pS.print(" - ");
             hWrite("</TD>");
           }
           hWrite("</TR>");
     ...
     private static void vgcomp (String y1, double x0, int t) {
       ...
       for (i=1; i<=saptvarg_sz; i++) {
         ...
         if (!html) pS.print(delim);
         pS.print(j[i]);
         delim = "\t";
       }
       pS.println();  hWrite("</TD></TR>");
     }
     /*---------------------------------------------------------------*/
     private static void vgcalc (int m) {
       String[][] tz = new String[33][8];
       int i, j, z;
       for (i=0; i<33; i++) for (j=0; j<8; j++) tz[i][j] = "    ";
       if (!html) {
         int s, p, q, r, u, v;
         for (i=0; i<=nplnt; i++) {
           s = varga[i][m];
         ...
         for (z=1; z<=33; z++) {
           if (z%8 == 1 && z!=17) {
             for (i=0; i<4; i++) pS.print("-------------");
             pS.println('-');
           } else if (z == 17)
             pS.println("--------------" + "       " +  V.vrg[m]
                     + "        " + "--------------");
           else if (z<9 || z>25) {
             for (i=0; i<8; i++) pS.print(
                  (i%2==0 ? "| " : " ") + tz[z][i] + " " );
             pS.println('|');
           } else {
             for (i=0; i<8; i++) pS.print(
         ...
         int maxGrh = 6;
         boolean found = false;
         for (i=0; i<=nplnt; i++) rItx[i] = 0;
         for (i=0; i<=nplnt; i++) {
           z = rIdx [ varga[i][m] - 1 ];
           tz[z][ rItx[z] ] =  V.graha[i];
           if (rItx[z] > maxGrh) maxGrh = rItx[z];
           rItx[z]++;
         }
         ...
         for (i=0; i<4; i++) hWrite(std+delm[i]+"</TD>");
         hWrite("</TR><TR>");
         for (i = 0; i < 12;  i++) {
           hWrite(std);
           found = false;
           ...
           if (!found) hWrite(delim);
           for (; j<=maxGrh/2; j++) hWrite("<BR>");
           if (i==4) {
     ...
     private static void vgprint (int detl) {
       if (detl == 0) {
         ...
         for (int m=0; m<=saptvarg_sz-4; m++) vgcalc(m);
     ...
     private static void ashtak () {
       ...
       pS.println();    hWrite("</TD></TR></TABLE><HR>");
       if (!html) pS.println(dashL);
       pageHeader();
     }
     /*---------------------------------------------------------------*/
     private static void vimsHeading (int detl) {
       ...
       */
       ...
       if (detl == 1) {
         pS.print("Dasa  ");        hWrite(sTDTDc);
         pS.print("   Bhukti ");    hWrite(sTDTDc);
         pS.print("  AntarDasa ");  hWrite(sTDTDc);
         pS.print("  Date    ");        hWrite(sTDTDc);
         //pS.print(" Month");        hWrite(sTDTDc);
         pS.println("         Age");
       } else {
         pS.print("Dasa  ");        hWrite(sTDTDc);
         pS.print("   Bhukti  ");   hWrite(sTDTDc + sTDTDc);
         pS.print("   Date    ");       hWrite(sTDTDc);
         //pS.print(" Month");        hWrite(sTDTDc);
         pS.println("         Age");
       }
       ...
       for (c=q; c <= q+8; c++) {
         ...
         for (d = 0; d<=8; d++) {
           ...
           if (detl == 0) {
             ...
             hWrite("<TR><TD>");
             pS.print( V.dL[c] + "  ");    hWrite("</TD><TD>");
             pS.print( V.dL[n] + "   ");   hWrite(sTDTDc + sTDTDc);
             /*
             pS.print(fmt.format(t) + "   ");hWrite(sTDTDc);
             pS.print(fmt.format(r) + "   ");hWrite(sTDTDc);
             pS.println(p);     hWrite("</TD></TR>");
             */
             pS.print(fmt.format(t)+"-"+V.mth[r-1]+"-"+p+" "); hWrite(sTDTDc);
             ...
             for (e=0; e<=8; e++) {
               ...
               hWrite("<TR><TD>");
               pS.print( V.dL[c]);           hWrite("</TD><TD>");
               pS.print("  " +  V.dL[n]);    hWrite("</TD><TD>");
               pS.print("  " +  V.dL[g]);       hWrite(sTDTDc);
               /*
               pS.print("   " + fmt.format(t)); hWrite(sTDTDc);
               pS.print("   " + fmt.format(r)); hWrite(sTDTDc);
...
/*
  ...
  */
       ...
       for (i=0; i <= nplnt; i++) vrg1[i] = varga[i][0];  // Raasi
       snE.vrg_1 = vrg1;
       int[] vrg9 = new int[nplnt+1];
       for (i=0; i <= nplnt; i++) vrg9[i] = varga[i][3];  // Navamsa
       snE.vrg_9 = vrg9;
     ...
     */
     /*---------------------------------------------------------------*/
     snJatak (PrintWriter pW, String args[])
     {
		pS = pW;
		snJatak(args);
     }
     ...
     public void snJatak (String args[])
     {
       ...
       try {
         ...
         if (!argsPassed) {
           DataInputStream dis = new DataInputStream(System.in);
           d=0; m=0; y=0; h=0; mt=0; sec=0; ew=' '; ns=' ';
           System.out.println("============================="+
                      "======================");
           ...
           System.out.println("Enter Name : "); System.out.flush();
           name = dis.readLine();
           System.out.println("Enter Sex : "); System.out.flush();
           sex = dis.readLine();
           System.out.println("Day (dd) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           d = Integer.parseInt(nxtline);
           System.out.println("Month (mm) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           m = Integer.parseInt(nxtline);
           System.out.println("Year (yyyy) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           y = Integer.parseInt(nxtline);
           System.out.println("Hours (HH) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           h = Integer.parseInt(nxtline);
           System.out.println("Minutes (MM) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           mt = Integer.parseInt(nxtline);
           System.out.println("Seconds (SS) : "); System.out.flush();
           nxtline = dis.readLine();
           sec = Double.parseDouble(nxtline);
           tob = (double)h + ((double)mt + ((double)sec / 60.0)) / 60.0;
           System.out.println("Time Zone (in Hours) : "); System.out.flush();
           nxtline = dis.readLine();
           timeZone = Double.parseDouble(nxtline);
           System.out.println("Place : "); System.out.flush();
           place = dis.readLine();
           System.out.println("Latitude (deg.) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           latdeg = Integer.parseInt(nxtline);
           System.out.println("Latitude (min.) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           latmt = Integer.parseInt(nxtline);
           ns = 'A'; ew = 'B';
           System.out.println("North/South (N/S) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           if (nxtline.length() > 0) {
           ...
           System.out.println("Longitude (deg.) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           longdeg = Integer.parseInt(nxtline);
           System.out.println("Longitude (min.) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           longmt = Integer.parseInt(nxtline);
           System.out.println("East/West (E/W) : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           if (nxtline.length() > 0) {
             nxtline.getChars(0,1,news,0);
           ...
           System.out.println("[0]-Basic [1]-Detail : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           detail = Integer.parseInt(nxtline);
           System.out.println("Ayanamsa :- ");
           System.out.println("[0]-N.C.Lahiri [1]-B.V.Raman " +
               "[2]-Chandra Hari [3]-Other : "); System.out.flush();
           nxtline = dis.readLine();
           nxtline = nxtline.trim();
           ayana_opt = Integer.parseInt(nxtline);
           if (ayana_opt > 2) {
             System.out.println("Ayanamsa (deg.) : "); System.out.flush();
             nxtline = dis.readLine();
             nxtline = nxtline.trim();
             aya_deg = Integer.parseInt(nxtline);
             System.out.println("Ayanamsa (mts.) : "); System.out.flush();
             nxtline = dis.readLine();
             nxtline = nxtline.trim();
             aya_mts = Integer.parseInt(nxtline);
             System.out.println("Ayanamsa (sec.) : "); System.out.flush();
             nxtline = dis.readLine();
             nxtline = nxtline.trim();
		   ...
		   if (outType == 'H' || outType == 'T') {
			 for (int kx=0; kx < 2; kx++) {
               try {
                 ...
                 pS = new PrintWriter(new FileOutputStream(pSFD));
			 ...
			 // PrintWriter is already Passed as Argument while calling this function
		   }
         ...
         lahiri_ayan = plnt[0][0] = ayan(0,y,m,d,h,mt,sec,aya_val);// Apply Lahiri's First
          ...
          hWrite("</TD></TR></TABLE>");
          hWrite("<TABLE><TR><TD>");
          pS.print("Name           : "); hWrite("</TD><TD colspan=2>");
          hWrite("<STRONG>");
          pS.print(name);              hWrite("</TD>");
          //hWrite("<TD width=75></TD><TD></TD><TD>");
          hWrite("<TD width=\"1%\"></TD><TD>");
          pS.print("\t\tSex  : ");    hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.println(sex);        hWrite("</TD></TR><TR><TD>");
          pS.print("Date of Birth  : "); hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.print(""+d+"-"+V.mth[(int)m-1]+"-"+y);
          hWrite("</TD>");   hWrite("<TD width=75>");
          hWrite("<STRONG>");
          //d_o_w = V.day[(int)j];
          snE.dow_No = (short)j;
          snE.dow = V.day[(int)j];
          ...
          hWrite("</TD><TD></TD><TD>");
          pS.print("Time of Birth  : "); hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.print(HMS.hms(tob));
          pS.println(" ("+(timeZone >= 0.0 ? "+":"-")+
              (timeZone >= 0.0 ? timeZone : -timeZone)+" GMT)");
          hWrite("</TD></TR></TABLE><TABLE><TR><TD>");
          pS.print("Place of Birth : "); hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.println(place);             hWrite("</TD></TR></TABLE><TABLE><TR><TD>");
          pS.print("Latitude       : "); hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.println(DMS.dms(lat) +" "+ns); hWrite("</TD>");
          hWrite("<TD width=75></TD><TD>");
          pS.print("Longitude      : "); hWrite("</TD><TD>");
          hWrite("<STRONG>");
          pS.println(DMS.dms(longt) +" "+ ew); hWrite("</TD></TR>");
          /*
          ...
          */
          ...
          hWrite("</a></TD></TR><TR><TD>");
          if (!html) pS.println(dashL); hWrite("</TD></TR>");
          hWrite("</TABLE>");
          hWrite("<TABLE border='1'><TR>"+sTDc);
          pS.print(" Graha ");      hWrite(sTDTDc);
          if (!html) pS.print("      ");
          if (html) {
            pS.print("Nirayana-Raasi"); hWrite(sTDTDc);
            pS.print("Sayana-Raasi");   hWrite(sTDTDc);
          }
          pS.print(" Nirayana ");   hWrite(sTDTDc);
          if (!html) pS.print(" (");
          pS.print("Sayana");  hWrite(sTDTDc);
          if (!html) pS.print(") ");
          if (!html) pS.print("  \t");
          if (!html) {
            pS.print("Raasi ");       hWrite(sTDTDc);
          }
          pS.print("(Paada)");      hWrite(sTDTDc);
          pS.print("Naksatra ");    hWrite(sTDTDc);
          pS.println(" Gati"); hWrite("</TD></TR><TR><TD>");
          if (!html) pS.println(dashL);   hWrite("</TD></TR>");
          prplnt();            hWrite("<TR><TD>");
          if (!html) pS.println(dashL);   hWrite("</TD></TR><TR><TD>");
          pS.println();        hWrite("</TD></TR></TABLE border=1>");
          bhav();
          saptavg();
